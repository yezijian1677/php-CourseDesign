...
$sql = "delete from `star`where `username`= '{$user['username']}' and `newsId` = '{$newsId}';";
if ($result = mysqli_query($con, $sql)){
    echo "<script>alert('取关成功');window.location.href='detail.php?id='+'{$newsId}';</script>";exit;
} else {
    echo "<script>alert('取关失败');window.location.href='index.php';</script>";exit;
}
?>